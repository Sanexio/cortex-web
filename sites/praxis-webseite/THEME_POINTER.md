...
## Aktueller Theme-Stand (Stand 2026-04-26 nach S45 Room-Slider + Dynamic-Height)

| Eintrag | Wert |
|---------|------|
| Commit-Hash | `901b602` (auf `ff3720a` → `6214b33` → `e3e0631` → `9907b7f` → `25662ad` → `c090173` → `e2336e2` → `c7acaf7` → `058b062` → `15cf331` → `70d1b29` → `ae9b1b8` folgend) |
| Commit-Message | `feat(s45-rooms): dynamic container height follows active slide (PXZ 2.7.53)` |
| PXZ_VERSION | `2.7.53` |
| S45-Bedeutung | Room-Slider auf beiden Standort-Pages aktiv. PXZ 2.7.52 (`ff3720a`) bringt den Slider selbst (3 neue Files: `template-parts/room-slider.php`, `assets/css/room-slider.css`, `assets/js/room-slider.js`); PXZ 2.7.53 (`901b602`) bringt Dynamic-Height (Container folgt aktivem Slide). `parse_blocks()` splittet `core/media-text`-Blöcke aus `post_content` in Slides; Postmeta-Toggle `pxz_use_room_slider = 1` aktiviert pro Page. Vanilla-JS-Carousel (Pfeile + Dots + Swipe + Keyboard, A11y). Container V1 (chalk-bg + 1px line + soft shadow) differenziert vom Adress-Container. Slider-Section in `.pxz-container` (1600 px) → matches address container width; Tail-Content max 1140 px Lesebreite. Hauptpraxis Page 9691: 10 neue Räume-Bilder uploaded (Media-IDs 9798–9834, Quellordner `Cortex-Web/_media-source/praxis/standorte/grueneburgweg/`). Zweigpraxis Page 9693: 2 Slides aus bestehendem media-text-Content (Testlauf). Spec `specs/sprint-2/S45_room-slider.md`, 10/10 AKs grün. 2 neue Patterns. |
| S2.3-diagnostik-Bedeutung | Cluster `diagnostik` Content-Migration (Session 21): eigener Top-Nav-Bereich + Hub `/diagnostik/` + Sub-Hub `/sonographie/` mit 3 Nested-Kindern (`schilddruese`, `beingefaesse`, `atlas`) + 3 flache Detail-Pages (`/belastungs-ekg/`, `/lungenfunktion/`, `/labor/`). 8 Pages migriert/inserted, 1 shell-Dublette gelöscht (4297), 1 konsolidiert (296+475 → `/labor/`), 1 gated auf draft (atlas, R-7 DSGVO pending). NEU `template-diagnostik-hub.php` (dual-mode: top-hub + sub-hub) + `assets/css/diagnostik-hub.css` + `inc/diagnostik-data.php` (SSoT mit `pxz_diagnostik_categories()` / `pxz_diagnostik_sono_children()` mit Draft-Filter / `pxz_is_diagnostik_uri()`). Nav-Erweiterung: neues 5tes Top-Item `Diagnostik ▼` mit 4 Submenu-Kindern (Sonographie, Belastungs-EKG, Lungenfunktion, Labor) in DE/EN/FR/ES + neues `match_prefix`-Feld + Helper `pxz_nav_matches_prefix()` für Active-State bei nested URIs. 8 neue SEO-Funktionen in `inc/seo-data.php` (MedicalProcedure / MedicalTest / ImageGallery für atlas). **In-Session-Bug-Fix:** `pxz_old_slug_fallback_redirect()` in functions.php (WP-Core-Redirect für Pages unzuverlässig — 4/5 alte Slugs lieferten 404 statt 301; Theme-Helper filtert auf `post_status=publish` und liefert zuverlässige 301). Mojibake-Fix bei Labor + Sonographie-Content. Cross-Links dicht (F6a): Lunge→Gesundheits-Check-Up, EKG→Cardio-Check-Up, Labor→Basic-Check. Migration idempotent (Lauf 2 = 0 Mutationen). 12/12 AK grün. Siehe `specs/sprint-2/evidence/2026-04-22_s2.3-diagnostik_self-check.md`. |
| S2.3-aerzte-services-Bedeutung | Cluster `aerzte` (8 Detail-Pages, alle wählbar — Dr.-Stracke-Constraint) + Cluster `services` (Konsolidierung). 8 Arzt-Detail-Pages auf `template-arzt.php`: `/dr-stracke/` mit Echt-Content aus Archive (Mojibake bereinigt, Werdegang/MBA/Studium kuratiert) + 7 Stub-Profile (Initialen-Avatar + intro aus `pxz_team_doctors()`). **Auf jeder Arzt-Page erscheint „Weitere Ärztinnen und Ärzte"-Sektion** mit 7 Mini-Cards der Kollegen → kein Arzt steht isoliert. `/team/`-Cards werden klickbar (`<a class="pxz-team-card-link">`). Brand-Switch Sanexio erweitert via `pxz_is_sanexio_uri()` Helper — gilt auf `/team/` UND allen 8 Arzt-Slugs (Header-Logo + `<title>` über `blogname`-Override). Menü-Label „Team" → „Ärzte" (4 Sprachen). NEU `template-leistungen.php` (Services-Hub mit 3-Sektionen-Card-Grid auf `/leistungen/`, ID 261 Slug+Content-Swap). `/impfungen/` konsolidiert aus 299+472, Mojibake-clean, 5 H2-Sektionen. `/rund-ums-impfen/`, `/corona-impfung/`, `/arzt-team/` → draft (archiviert). Helpers in `inc/team-data.php`: `profile_url`, `pxz_doctor_slugs()`, `pxz_doctor_by_slug()`, `pxz_is_sanexio_uri()`, `pxz_render_other_doctors()`. 3 neue SEO-Funktionen in `inc/seo-data.php`: `pxz_seo_data_leistungen()`, `pxz_seo_data_impfungen()`, generische `pxz_seo_data_doctor($slug)` für 8 Personen (schema.org/Physician). 12/12 AK = 100 %. Migration idempotent. Verify+smoke+validate alle grün. Siehe `specs/sprint-2/evidence/2026-04-22_s2.3-aerzte-services_self-check.md`. |
| S2.4-Bedeutung | Menü-Restrukturierung (Dr.-Stracke-Direktive 2026-04-22): kuratierte Top-Navigation mit 7 Items (Praxis · Team · Fachrichtungen · Check-Ups ▼ · Sprechstunden · Kontakt · Karriere) und 5-Kinder-Submenu `Check-Ups ▼`. Neue Daten-Quelle `inc/nav-data.php` (F1b: PHP-Array, 4 Sprachen). Mobile-Burger + Off-Canvas-Drawer + Accordion via `<details>` (Progressive Enhancement). Desktop-Submenu pure-CSS-Hover mit `:focus-within`. Active-State `.is-active` + `.is-active-parent` + `aria-current="page"`. `/aerzte/` aus Menü entfernt (Cluster noch nicht migriert — Direktive „Menü wächst mit Content"). `/kontakt/` → direkt `/contact-us/` (kein 301). Stub-Page `/fachrichtungen/` auf `template-standard.php` via Cortex-Web-Migration angelegt. Footer NICHT im Scope (→ S2.4b). Brand-Switch `/team/` → Sanexio-Logo weiterhin funktional. Home-Hero visuell unverändert (Template + homepage-data.php unberührt). 12/12 AK grün. Siehe `specs/sprint-2/evidence/2026-04-22_s2.4_self-check.md`. |
...
### Versionskette des Themes (Stand 2026-04-22, Ende S2.3-diagnostik)

```
25662ad  feat(s2.3-diagnostik): cluster hub + 4 top categories + 3 nested sono children (PXZ 2.7.18)
c090173  feat(s2.3-aerzte-services): 8 doctor profiles + services hub + impf consolidation (PXZ 2.7.17)
e2336e2  feat(s2.4): top-nav restructure + mobile drawer + Check-Ups submenu (PXZ_VERSION 2.7.16)
