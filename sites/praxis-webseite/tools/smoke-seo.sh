...
echo "--- /praxis/"
URL="$BASE_URL/praxis/"
assert_grep  "$URL" "<title>Internist Frankfurt Westend – Praxiszentrum Dr\\. Stracke und Kollegen</title>" "title tag"
...
assert_grep  "$URL" "Internistische Schwerpunktpraxis mit hausärztlicher Grundversorgung" "h1 content"

# --- /team/ -----------------------------------------------------------------
echo ""
echo "--- /team/"
URL="$BASE_URL/team/"
assert_grep  "$URL" "<title>Unser Team – Praxisgemeinschaft Sanexio Frankfurt</title>" "title tag"
assert_grep  "$URL" "<meta name=\"description\" content=\"Die Ärztinnen und Ärzte der Praxisgemeinschaft Sanexio" "meta description"
...
assert_count "$URL" "<h1[^>]*>" 1 "exactly 1 <h1>"
assert_grep  "$URL" "<h1[^>]*>Unser Team – Menschen, die Ihre Gesundheit ins Zentrum stellen</h1>" "h1 content"
