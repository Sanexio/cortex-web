...
- **Version:** `0.9.15` — Stand 2026-05-08 nach **Welle S55b-d (Polish-Phase: Footer-Right-Size + Karriere-Linksbündig + C-Token-Container-System)** + **GitHub-Mirror-Setup**. Cortex-Web-HEAD `f6df0af` (Theme-Audit-Tool + .gitignore). Praxis-Theme-HEAD `cad2ff2` (PXZ 2.7.179, 25 Files). **Beide Repos jetzt auf GitHub privat:** `Sanexio/cortex-web` + `Sanexio/praxiszentrum-theme`. **Detail-Chronik in `sites/praxis-webseite/SESSION_RESUME.md` §1** (Single-Source LL-053d).
